 ...
 * Only includes the methods actually used by SeamsWeb
 *
...
import { decodeJsonRpcEnvelope, type JsonRpcEnvelope } from '@shared/utils/jsonRpc';
import {
...
export class MinimalNearClient implements NearClient {
  ...
  private unwrapRpcResult(rpc: JsonRpcEnvelope, operationName: string): unknown {
    if (rpc.kind === 'failure') {
      throw NearRpcError.fromRpcError(operationName, rpc.error);
    }
