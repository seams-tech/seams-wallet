      ...
      <div className="seams-transaction-page">
        <fieldset className="seams-transaction-group" disabled={disableAll}>
          ...
          <div className="seams-transaction-choices">
            {UI_CHOICES.map(renderUiChoice.bind(null, currentConfirmConfig.uiMode ?? 'modal', onSetUiMode))}
          </div>
        </fieldset>
        <fieldset className="seams-transaction-group" disabled={disableAll || disableRequireClick}>
          <legend>Confirmation step</legend>
          <p>{disableRequireClick ? 'Choose Modal or Drawer to configure the confirmation step.' : 'Choose whether to click Continue before authentication starts.'}</p>
          <div className="seams-transaction-choices seams-transaction-choices--two">
            {BEHAVIOR_CHOICES.map(renderBehaviorChoice.bind(null, currentConfirmConfig, onSetDelay, onToggleSkipClick))}
          </div>
        </fieldset>
      </div>
    );
...
const UI_CHOICES: UiChoice[] = [
  { value: 'none', label: 'No review', description: 'Start authentication without showing a review panel.' },
...
const BEHAVIOR_CHOICES: BehaviorChoice[] = [
  { value: 'skipClick', label: 'Start automatically', description: 'Begin authentication as soon as the review opens.' },
  { value: 'requireClick', label: 'Click to continue', description: 'Review the details, then click Continue to start authentication.' },
];
...
function renderUiChoice(
  ...
  return (
    <button type="button" key={choice.value} className="seams-transaction-choice"
      aria-pressed={selected === choice.value} onClick={onSelect?.bind(null, choice.value)}>
...
function setBehavior(
  ...
  if (value === 'skipClick' && config.autoProceedDelay !== 0) setDelay(0);
...
function renderBehaviorChoice(
  ...
  return (
    <button type="button" key={choice.value} className="seams-transaction-choice"
      aria-pressed={selected === choice.value} onClick={setBehavior.bind(null, config, setDelay, toggle, choice.value)}>
      <span className="seams-transaction-choice-indicator" aria-hidden="true" />
      <strong>{choice.label}</strong>
      <span>{choice.description}</span>
    </button>
